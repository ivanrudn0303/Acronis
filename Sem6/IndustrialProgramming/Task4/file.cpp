...
//у сложения больший приоритет чем у сдвига
...
{
//new выбрасывает исключение, а не NULL
...
//сравнение с указателем на строку
	while( color <= PG_COLOR_RED )
	{
//%s для строки а не числа, %u для const char* выведет адрес 
		printf("PgStrg[(%s) %u] ********** \n", color, PgColorName[color] );
//пропускает первый элемент
		for( PageDesc* Pg = PageStrg[++color]; Pg != NULL; Pg = Pg->next )
		{
//no uAddr in PageDesc
			if( Pg->uAddr = NULL )
				continue;
//нельзя вывести аттрибут uKey(union)
